...
class SecurityAnalyzer {
  ...
  private identifyVulnerabilities(): SecurityVulnerability[] {
    ...
    const hasAuthService = this.nodes.some(
      (node) =>
    ...
    if (!hasAuthService && this.nodes.length > 2) {
      vulnerabilities.push({
        ...
        description:
          "No dedicated authentication service found in the architecture",
    ...
    const hasGateway = this.nodes.some(
      (node) => (node.data as NodeData).config.type === "gateway"
    );
    ...
    this.edges.forEach((edge) => {
      ...
      if (sourceNode && targetNode) {
        ...
        const communicationType = edgeData?.communicationType;

        // Check if edge is secure based on type or communication type
        const isSecureEdge =
          edgeType === "https" ||
          edgeType === "tls-async" ||
          edgeType === "encrypted-data" ||
          communicationType === "https" ||
          communicationType === "tls-async" ||
          communicationType === "encrypted-data" ||
          edgeType?.includes("tls") ||
          edgeType?.includes("https") ||
          edgeType?.includes("encrypted");
        ...
        if (!isSecureEdge) {
          vulnerabilities.push({
            ...
            severity: "high",
            type: "encryption",
            service: `${(sourceNode.data as NodeData).config.name} → ${
              (targetNode.data as NodeData).config.name
            }`,
            ...
            remediation: [
              "Enable TLS/SSL encryption using HTTPS edge",
              "Use TLS Async edge for asynchronous communication",
              "Use Encrypted Data edge for sensitive data flows",
    ...
    this.nodes.forEach((node) => {
      ...
      if (config.type === "external") {
        vulnerabilities.push({
          ...
          description:
            "Dependency on external service without security assessment",
          impact:
            "Potential data leakage, service disruption, supply chain attacks",
          remediation: [
    ...
    //       description:
    //         "Database encryption status is not explicitly configured",
    ...
    if (hasAuthService) {
      const authNodes = this.nodes.filter(
        (node) =>
          (node.data as NodeData).config.type === "auth" ||
          (node.data as NodeData).config.name.toLowerCase().includes("auth")
      );
  ...
  private performSecurityChecks(): SecurityCheck[] {
    ...
    checks.push({
      ...
      recommendation: hasGateway
        ? undefined
        : "Consider implementing API Gateway for centralized authorization",
    });

    // Encryption checks
    const encryptedEdges = this.edges.filter((e) => {
      const edgeType = (e as any).type;
    ...
    const encryptionRatio =
      this.edges.length > 0 ? encryptedEdges.length / this.edges.length : 1;
    checks.push({
      id: "encryption",
      ...
      status:
        encryptionRatio >= 0.8
          ...
          ? "warning"
          : "fail",
      description: `${(encryptionRatio * 100).toFixed(
        1
    ...
    checks.push({
      ...
      status:
        externalServices.length === 0
          ? "pass"
          : externalServices.length <= 2
          ? "warning"
          : "fail",
      description: `${externalServices.length} external service dependencies`,
      recommendation:
        externalServices.length > 0
          ? "Review and secure external service integrations"
          : undefined,
    });

    // // Data protection checks
    // const databases = this.nodes.filter(
    //   (n) => (n.data as NodeData).config.type === "database"
    // );
    // checks.push({
  ...
  private generateThreatModel(): ThreatModel[] {
    ...
    // Data breach threat
    const sensitiveServices = this.nodes.filter(
      (n) =>
        ["database", "auth", "payment"].includes(
          (n.data as NodeData).config.type
        ) || (n.data as NodeData).config.name.toLowerCase().includes("user")
    );

    if (sensitiveServices.length > 0) {
      // Check if sensitive services have insecure connections
      ...
      sensitiveServices.forEach((service) => {
        ...
        connectedEdges.forEach((edge) => {
          ...
          if (!isSecureEdge) {
            const otherNodeId =
              edge.source === service.id ? edge.target : edge.source;
            const otherNode = this.nodes.find((n) => n.id === otherNodeId);
            if (otherNode) {
              insecureConnections.push(
                `${(service.data as NodeData).config.name} ↔ ${
                  (otherNode.data as NodeData).config.name
                }`
              );
            }
          }
      ...
      // Adjust threat based on whether sensitive services have secure connections
      const hasInsecureConnections = insecureConnections.length > 0;
      const likelihood = hasInsecureConnections ? "high" : "medium";
      const riskRating = hasInsecureConnections ? 20 : 15;
      ...
      if (hasInsecureConnections) {
        mitigations.unshift(
          "Use secure edge types (HTTPS, TLS Async, Encrypted Data) for sensitive service connections"
        );

        threats.push({
          id: "data-breach",
          threat: "Data Breach",
          description: hasInsecureConnections
            ? "Unauthorized access to sensitive data through compromised services with insecure connections"
            : "Unauthorized access to sensitive data through compromised services",
          likelihood,
          impact: "very-high",
          riskRating,
          mitigations,
          affectedAssets: [
            ...sensitiveServices.map((s) => (s.data as NodeData).config.name),
            ...(hasInsecureConnections
              ? [`Insecure connections: ${insecureConnections.join(", ")}`]
              : []),
          ],
        });
      }
    ...
    if (publicServices.length > 0) {
      threats.push({
        ...
        description:
          "Overwhelming services with malicious traffic to cause service disruption",
        likelihood: "medium",
        impact: "high",
        riskRating: 12,
        mitigations: [
        ...
        affectedAssets: publicServices.map(
          (s) => (s.data as NodeData).config.name
        ),
      });
    }
    ...
    if (unencryptedConnections.length > 0) {
      threats.push({
        ...
        description:
          "Interception and manipulation of unencrypted communications",
        likelihood: "low",
        impact: "high",
        riskRating: 8,
        mitigations: [
        ...
        affectedAssets: unencryptedConnections.map((e) => {
          ...
          return `${(source?.data as NodeData)?.config.name} → ${
            (target?.data as NodeData)?.config.name
          }`;
        }),
      });
    }
    ...
    if (this.nodes.length > 5) {
      threats.push({
        ...
        description:
          "Malicious or accidental actions by internal users with system access",
        likelihood: "low",
        impact: "high",
        riskRating: 8,
        mitigations: [
    ...
    if (externalServices.length > 0) {
      threats.push({
        ...
        affectedAssets: externalServices.map(
          (s) => (s.data as NodeData).config.name
        ),
      });
    }
  ...
  private assessAuthenticationControls(): number {
    ...
    if (authNodes.some((n) => (n.data as NodeData).config.replicas > 1))
      score += 20;
